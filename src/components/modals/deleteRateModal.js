...
function DeleteRateModal({currentRate, editRateModal}) {
    const [rate, setrate] = useState(currentRate?.rate)
    const [showModal, setshowModal] = useState(false)

    useEffect(() => {
        setshowModal(editRateModal)
    }, [])
        ...
        <div className="relative w-full max-w-2xl px-4 h-full md:h-auto m-auto">
            ...
            <div className="bg-blue-50 rounded-lg shadow relative dark:bg-gray-700 w-2/3 m-auto">
                ...
                <div className="flex space-x-2 items-center p-6 border-t border-gray-200 rounded-b dark:border-gray-600">
                    <button data-modal-toggle="default-modal" type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={updateRate}>Update</button>
